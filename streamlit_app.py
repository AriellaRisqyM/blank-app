...
def full_preprocess_and_label_df(_df, text_column, _kamus_normalisasi, _stop_words, _stemmer, _senti_dict, _sorted_idioms, _negating_words, _question_words):
    ...
    df_processed['cleaned_text'] = df_processed[text_column].astype(str).apply(clean_text)
    ...
    # Pengecekan NLTK sudah di STEP 1, jadi bisa langsung pakai word_tokenize
    try:
        # Gunakan word_tokenize yang sudah diimpor secara global
        df_processed['tokenized_text'] = df_processed['case_folded_text'].progress_apply(word_tokenize)
    except NameError:
         # Ini seharusnya tidak terjadi jika impor di STEP 1 benar
         st.error("Error: Fungsi 'word_tokenize' tidak terdefinisi. Periksa impor NLTK di awal script.")
         return pd.DataFrame()
    except Exception as e:
...
# ==============================================================================
# ML Modeling Function (Dengan Tuning, Tanpa SMOTE - Tetap Sama)
# ==============================================================================
# (Fungsi train_evaluate_tuned_models_no_smote tetap sama seperti sebelumnya)
...
# ==============================================================================
# Single Text Processing Function (Tetap Sama)
# ==============================================================================
# (Fungsi process_single_text tetap sama)
...
# ==============================================================================
# Helper Function for Download (Tetap Sama)
# ==============================================================================
# (Fungsi convert_df_to_csv tetap sama)
@st.cache_data
def convert_df_to_csv(df):
   return df.to_csv(index=False).encode('utf-8')

# ==============================================================================
# Word Cloud Function (Tetap Sama)
# ==============================================================================
...
# --- Load Resources ---
# Pindahkan pemanggilan fungsi load ke sini agar hanya dijalankan sekali saat app start
# Tidak perlu spinner di sini karena @st.cache_resource menangani caching
...
st.success("✅ Sumber daya siap.") # Pindahkan pesan ini setelah semua resource dimuat
st.markdown("---")
...
with tab1:
    ...
    if uploaded_file is not None:
        ...
        if uploaded_file.name != st.session_state.current_filename:
            ...
            # Reset pilihan kolom jika file baru
            if 'selectbox_column' in st.session_state:
                del st.session_state['selectbox_column']
        ...
        try:
            ...
            with st.spinner(f"Membaca file {uploaded_file.name}..."):
                if uploaded_file.name.endswith('.csv'):
                    try: df_input = pd.read_csv(uploaded_file)
                        ...
                        uploaded_file.seek(0); df_input = pd.read_csv(uploaded_file, encoding='latin1')
            ...
            col_index = 0
            # Cek apakah state ada DAN value-nya valid di kolom saat ini
            ...
            else: # Jika tidak valid (misal file baru), reset state
                 if 'selected_text_column' in st.session_state:
                      del st.session_state['selected_text_column']

            # Definisikan callback untuk update state
            def update_selected_column():
                 st.session_state.selected_text_column = st.session_state.selectbox_column
            ...
            text_column = st.selectbox(
                 ...
                 on_change=update_selected_column # Panggil callback saat berubah
            )
            ...
            if text_column:
                ...
                if st.button("🔬 Proses Teks (Stemming) & Label Lexicon", key="button_process_lexicon"):
                    ...
                    if text_column not in df_input.columns: st.error("Kolom tidak valid.")
                        ...
                        if df_valid.empty: st.warning("Tidak ada baris dengan teks valid setelah pembersihan awal.")
                            ...
                            # Spinner dikontrol di dalam fungsi full_preprocess_and_label_df
                            ...
                            else: st.session_state.processed_df = df_processed_labeled

                # --- Display Preprocessing & Labeling Results ---
                # Cek jika dataframe hasil proses ada di session state
                if st.session_state.processed_df is not None and not st.session_state.processed_df.empty:
                    ...
                    st.write("Dibuat dari 'case_folded_text' (sebelum stemming).")
                    ...
                    st.warning("Model ML (NB, SVM Tuned, LR Tuned) akan dilatih menggunakan 'sentiment' hasil lexicon sebagai target dan 'stemmed_joined_text' sebagai fitur.")
                    ...
                    if st.session_state.ml_results is not None:
                        ...
                        with col_m1: st.metric("Akurasi Naive Bayes", f"{results_ml.get('naive_bayes', {}).get('accuracy', 0):.2%}")
                        with col_m2: st.metric("Akurasi SVM (Tuned)", f"{results_ml.get('svm_tuned', {}).get('accuracy', 0):.2%}", help=f"CV Score: {results_ml.get('svm_tuned', {}).get('cv_score', 0):.2%}")
                        with col_m3: st.metric("Akurasi Log. Reg. (Tuned)", f"{results_ml.get('logistic_regression_tuned', {}).get('accuracy', 0):.2%}", help=f"CV Score: {results_ml.get('logistic_regression_tuned', {}).get('cv_score', 0):.2%}")

                        with st.expander("Lihat Laporan Klasifikasi Detail"):
                            if 'naive_bayes' in results_ml: st.text("Naive Bayes:"); st.dataframe(pd.DataFrame(results_ml['naive_bayes']['report']).transpose())
                            if 'svm_tuned' in results_ml: st.text("SVM (Tuned):"); st.dataframe(pd.DataFrame(results_ml['svm_tuned']['report']).transpose())
                            if 'logistic_regression_tuned' in results_ml: st.text("Logistic Regression (Tuned):"); st.dataframe(pd.DataFrame(results_ml['logistic_regression_tuned']['report']).transpose())
                        ...
                        models_to_plot = {
                            ...
                            'Log. Reg. (Tuned)': ('logistic_regression_tuned', 'Oranges', axes_cm[2])
                        }
                        # Ambil labels dari hasil pertama yang tersedia
                        display_labels = next((res['labels'] for res in results_ml.values() if 'labels' in res), ['negatif', 'netral', 'positif'])

                        for model_name, (key, cmap, ax) in models_to_plot.items():
                            if key in results_ml:
                                try: # Tambahkan try-except untuk plot CM
                                     ...
                                     st.warning(f"Gagal membuat CM untuk {model_name}: {plot_err}")
                                     ax.set_title(f"{model_name}\nPlot Gagal")
                            else:
