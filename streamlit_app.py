...
def preprocess_and_label(_df, text_col, _pos_lex, _neg_lex, _kamus_norm, _stop_words, _stemmer):
    ...
    def update_progress(text):
        ...
        current_step += 1
        # Memperbarui progress bar di UI Streamlit
...
def train_models(_df_processed, max_features=5000, test_size=0.2): # Argumen ditambahkan
    ...
    # Menggunakan max_features dari UI
...
```Tentu, ini adalah kode Streamlit lengkap yang menggabungkan semua permintaan Anda:

1.  Menggunakan **alur preprocessing detail** dari `.ipynb` (Termasuk NLTK Tokenize, Normalize, Stopwords, dan Stemming).
2.  Menggunakan **keyword filter lengkap** dari Streamlit.
3.  Menggunakan **leksikon Fajri91 saja** untuk pelabelan.
4.  Menggunakan **input `stemmed_tokens`** dan logika `>=` untuk pelabelan (sesuai .ipynb).
5.  Menggunakan **input `case_folded_text`** untuk TF-IDF (sesuai .ipynb).
6.  Menggunakan model `MultinomialNB` (default) dan `LinearSVC` (sesuai .ipynb).
7.  Menambahkan **persentase berjalan** (progress bar) untuk preprocessing (Blok 5).
8.  Menambahkan **kontrol UI** untuk `Test Size` dan `Max Features` (Blok 9, di dalam Tab 1).

Kode ini juga menyertakan perbaikan untuk `nltk.download('punkt_tab')` dan error `StreamlitAPIException` (progress bar) yang Anda laporkan.
...
def preprocess_and_label(_df, text_col, _pos_lex, _neg_lex, _kamus_norm, _stop_words, _stemmer):
    ...
    # *** BAGIAN PERSENTASE BERJALAN ***
    progress_bar = st.progress(0, text="Memulai...")
    total_steps = 9 # (Clean, Dedupe, Fold, Filter, Token, Norm, Stopword, Stem, Label)
    current_step = 0
